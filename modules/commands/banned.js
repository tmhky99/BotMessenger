    ...
    case "unbanthread":
      {
        ...
        return api.sendMessage(`[${myString}]\n✅Bot đã gỡ ban Box!`, threadID);
        break;
      }
    ...
    case 'unbanuser':
      {
        ...
        api.sendMessage(`${myString}\n✅Bot đã gỡ ban người dùng!`, threadID);
        break;
      }
  }
};
    ...
    case "thread":
      {
        ...
        return api.sendMessage(listBanned.length != 0 ? api.sendMessage(`❎Hiện tại đang có ${listBanned.length} nhóm bị ban\n${listBanned.join("\n")}` +
          "\n☺️Reply tin nhắn này + số thứ tự để unban thread tương ứng",
          threadID, (error, info) => {
          ...
          messageID
        ) : "☺️Hiện tại không có nhóm nào bị ban!", threadID, messageID);
      }
    ...
    case "user":
      {
        ...
        return api.sendMessage(listBanned.length != 0 ? api.sendMessage(`❎Hiện tại đang có ${listBanned.length} người dùng bị ban\n${listBanned.join("\n")}` +
          "\n☺️Reply tin nhắn này + số thứ tự để unban user tương ứng",
          threadID, (error, info) => {
            client.handleReply.push({
              name: this.config.name,
              messageID: info.messageID,
          ...
          messageID
        ) : "☺️Hiện tại không có người dùng bị ban", threadID, messageID);
      }
